...
import { eq, and, count, isNull, lt, sql, ne } from "drizzle-orm";
...
router.get("/api/me/control-widgets", isAuthenticated, async (req: any, res) => {
  try {
    ...
    try {
      const [r] = await db.select({ val: count() }).from(users).where(and(eq(users.isActive, true), isNull(users.deletedAt)));
      totalStaff = Number(r?.val || 0);
    } catch {}

    try {
      const [r] = await db.select({ val: count() }).from(tasks).where(and(eq(tasks.status, "pending"), isNull(tasks.deletedAt)));
      pendingTasks = Number(r?.val || 0);
    } catch {}

    try {
      const today = new Date().toISOString().split("T")[0];
      const [r] = await db.select({ val: count() }).from(tasks).where(and(eq(tasks.status, "pending"), isNull(tasks.deletedAt), sql`${tasks.dueDate}::date < ${today}::date`));
      overdueTasks = Number(r?.val || 0);
    } catch {}

    try {
      const [r] = await db.select({ val: count() }).from(equipmentFaults).where(eq(equipmentFaults.status, "open"));
      openFaults = Number(r?.val || 0);
    } catch {}

    let resolvedFaults = 0;
    try {
      const [r] = await db.select({ val: count() }).from(equipmentFaults).where(eq(equipmentFaults.status, "resolved"));
    ...
    try {
      const [r] = await db.select({ val: count() }).from(notifications).where(and(sql`${notifications.userId}::int = ${user.id}`, eq(notifications.isRead, false)));
    ...
    // ── Staff breakdown ──
    let activeStaff = 0, leaveStaff = 0;
    try {
      const [r] = await db.select({ val: count() }).from(users).where(and(eq(users.isActive, true), isNull(users.deletedAt)));
    ...
    res.json({
      ...
      equipment: {
        ...
        resolutionRate: openFaults + resolvedFaults > 0 ? Math.round((resolvedFaults / (openFaults + resolvedFaults)) * 100) : 0,
      },
      training: {
        completion: 78,
        categories: {
    ...
    console.error("[control-widgets] Error:", error);
    res.json({
