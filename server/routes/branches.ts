    ...
    // ═══ HER CHECK-IN'DE shift_attendance OLUŞTUR (geç kalma fark etmez) ═══
    {
      ...
      if (!shiftIdForAttendance) {
        try {
          ...
          if (adHocShift) shiftIdForAttendance = adHocShift.id;
        } catch (adHocErr) {
          console.warn("[BRANCH-KIOSK] Ad-hoc shift creation failed:", adHocErr);
        }
      }
      ...
      if (shiftIdForAttendance) {
        try {
          ...
          console.warn("[BRANCH-KIOSK] shift_attendance write failed (non-blocking):", saErr);
        }
      }
    }
