...
router.post("/api/branches/:branchId/kiosk/shift-start", isKioskOrAuthenticated, async (req, res) => {
  try {
    ...
    if (isLateArrival) {
      // attendance_penalties requires shiftAttendanceId (NOT NULL FK via shifts → shift_attendance)
      // Primary anchor: use plannedShift; fallback: find any of today's shifts for this user
      let shiftIdForPenalty: number | null = plannedShiftId;

      if (!shiftIdForPenalty) {
        // Fallback: find any scheduled shift for this user today on any branch
      ...
      if (shiftIdForPenalty) {
        try {
          // Find or create shiftAttendance for this shift to anchor the penalty
          ...
          console.warn("[BRANCH-KIOSK] Late arrival penalty write failed (non-blocking):", penaltyErr);
        }
      } else {
        // No shift found — create ad-hoc shift so penalty can be tracked
        try {
          ...
          if (adHocShift) {
            ...
            await db.insert(attendancePenalties).values({
              shiftAttendanceId: newSA.id,
              type: 'late_arrival',
              minutes: lateMinutes,
              reason: `Kiosk giriş (plansız vardiya): ${lateMinutes} dk geç`,
              autoGenerated: true,
            });
            console.log(`[BRANCH-KIOSK] Ad-hoc shift created for unplanned attendance, penalty written: ${lateMinutes}m`);
          }
        } catch (adHocErr) {
          console.warn(`[BRANCH-KIOSK] Ad-hoc shift/penalty creation failed (non-blocking):`, adHocErr);
        }
      }
    }
...
router.post('/api/hq/kiosk/shift-start', isKioskAuthenticated, async (req, res) => {
  try {
    ...
    if (hqIsLateArrival) {
      ...
      let hqShiftIdForPenalty: number | null = hqPlannedShift?.id ?? null;

      if (!hqShiftIdForPenalty) {
        const [fallbackShift] = await db.select({ id: shifts.id })
          .from(shifts)
          .where(and(
            eq(shifts.assignedToId, userId),
          ...
          .limit(1);
        if (fallbackShift) hqShiftIdForPenalty = fallbackShift.id;
      }
      ...
      if (hqShiftIdForPenalty) {
        try {
          const [existingSA] = await db.select().from(shiftAttendance)
            .where(and(
              eq(shiftAttendance.shiftId, hqShiftIdForPenalty),
              eq(shiftAttendance.userId, userId)
            )).limit(1);
          ...
          if (existingSA) {
            ...
            const [newSA] = await db.insert(shiftAttendance).values({
              shiftId: hqShiftIdForPenalty,
              userId,
              checkInTime: now,
              status: 'present',
              latenessMinutes: hqLateMinutes,
            }).returning();
            saId = newSA.id;
          }
          ...
          await db.insert(attendancePenalties).values({
            shiftAttendanceId: saId,
            type: 'late_arrival',
            minutes: hqLateMinutes,
            reason: `Vardiyaya geç giriş: ${hqLateMinutes} dakika (HQ kiosk)`,
            autoGenerated: true,
          });
        } catch (penErr: unknown) {
          console.warn("[HQ-KIOSK] Late-arrival penalty write failed (non-blocking):", penErr instanceof Error ? penErr.message : String(penErr));
        }
      } else {
        // No shift — create ad-hoc for penalty tracking
        try {
          ...
          const [adHocShift] = await db.insert(shifts).values({
            ...
            shiftType: "adhoc", status: "completed",
          }).returning();
          if (adHocShift) {
            const [newSA] = await db.insert(shiftAttendance).values({
              shiftId: adHocShift.id, userId, checkInTime: now, status: 'present', latenessMinutes: hqLateMinutes,
            }).returning();
            await db.insert(attendancePenalties).values({
              shiftAttendanceId: newSA.id, type: 'late_arrival', minutes: hqLateMinutes,
              reason: `HQ Kiosk giriş (plansız): ${hqLateMinutes} dk geç`, autoGenerated: true,
            });
          }
        } catch (e) { console.warn("[HQ-KIOSK] Ad-hoc shift/penalty failed:", e); }
      }
    }
