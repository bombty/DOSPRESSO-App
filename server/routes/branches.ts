  ...
  try {
    ...
    if (isLateArrival) {
      ...
      if (shiftIdForPenalty) {
        ...
        // No shift found anywhere for this user today — DB FK constraint prevents penalty write
        console.warn(`[BRANCH-KIOSK] Late arrival (${lateMinutes}m) for user ${userId}: no shift scheduled today, penalty cannot be written (attendance_penalties requires shift_attendance FK)`);
      }
    }
  ...
  try {
    ...
    if (hqIsLateArrival) {
      ...
      if (hqShiftIdForPenalty) {
        ...
        console.warn(`[HQ-KIOSK] Late arrival (${hqLateMinutes}m) for user ${userId}: no shift scheduled today, penalty cannot be written (attendance_penalties requires shift_attendance FK)`);
      }
    }
