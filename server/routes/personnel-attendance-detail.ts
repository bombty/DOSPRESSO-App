...
router.get('/api/personnel/:userId/attendance-detail', isAuthenticated, async (req: any, res: Response) => {
  try {
    ...
    const shifts = await db.select({
      ...
      shiftStart: shiftAssignments.shiftStart,
      shiftEnd: shiftAssignments.shiftEnd,
      shiftType: shiftAssignments.shiftType,
      status: shiftAssignments.status,
    })
      .from(shiftAssignments)
      .where(and(
        eq(shiftAssignments.userId, targetUserId),
        gte(shiftAssignments.shiftDate, startDate),
        lte(shiftAssignments.shiftDate, endDate),
      ))
      .orderBy(desc(shiftAssignments.shiftDate))
      .catch(() => []); // shift_assignments yoksa boş dön
