...
export class DatabaseStorage implements IStorage {
  ...
  async calculateAndApplyPenalties(shiftAttendanceId: number): Promise<ShiftAttendance | undefined> {
    ...
    const totalWorkedMinutes = attendance.totalWorkedMinutes || 0;
    ...
    if (penalties.length > 0) {
      await db.insert(attendancePenalties).values(penalties);
    }
    
    const [updated] = await db.update(shiftAttendance)
      .set({
        latenessMinutes,
        earlyLeaveMinutes,
  ...
  async createManualPenalty(data: InsertAttendancePenalty): Promise<AttendancePenalty> {
    const [penalty] = await db.insert(attendancePenalties)
    ...
    if (attendance) {
      ...
      const currentEffective = attendance.effectiveWorkMinutes || 0;
      const currentCompliance = attendance.complianceScore || 100;
      
      await db.update(shiftAttendance)
        .set({
          penaltyMinutes: currentPenalty + data.minutes,
          effectiveWorkMinutes: Math.max(0, currentEffective - data.minutes),
  ...
  async checkSLABreaches(): Promise<void> {
    ...
    await db.update(guestComplaints)
      ...
      .where(and(
        ...
        sql`${guestComplaints.responseDeadline} < ${now.toISOString()}`
      ));
  }
