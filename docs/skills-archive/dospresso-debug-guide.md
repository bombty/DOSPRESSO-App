...
## Pilot Hazırlık 2026 — Yeni Bug Patterns (18-19 Nis)

### "column u.name does not exist" (HTTP 500)
**Root cause:** SQL'de tablo kolonu varsayımı. Drizzle schema-XX.ts'te kolonu doğrulamadan yazma.
**Fix:** Schema'yı önce grep ile gör:
```bash
grep -A 30 "export const users = pgTable" shared/schema/schema-02.ts
```
**Örnek:** `users` tablosunda `name` YOK, gerçek: `first_name + last_name + username`. Doğru SQL:
```sql
COALESCE(u.first_name || ' ' || u.last_name, u.username, 'Bilinmiyor') AS user_name
```

### "Missing pdks_record" — UTC ↔ TR Timezone Sınır Geçişi
**Root cause:** `now.toISOString().split('T')[0]` UTC tarih verir. TR saat sınırını aşan check-in'lerde yanlış güne yazar.
**Örnek:** Basri Şen 29 Mart 21:13 UTC = TR 30 Mart 00:13. Eski kod `recordDate='2026-03-30'` yazar, ama `shifts.shift_date='2026-03-29'` → JOIN tutmaz.
**Fix:** `import { trDateString } from "../lib/datetime"` kullan (Sprint D'de eklenmiş).

### "Silent failure / non-blocking error swallow"
**Root cause:** `try { ... } catch (e) { console.error(...); }` pattern stdout'a yazar ama admin görmez. Kullanıcı "check-in başarılı" sanır.
**Tespit:** 18-19 Nis Sprint D — 3 ay boyunca PDKS pipeline kopuk olduğunu kimse fark etmemiş.
**Fix:** `critLog()` helper ile DB'ye persist et. Sprint E'de eklenmiş:
```typescript
import { critLog } from "../lib/crit-log";
} catch (pdksErr) {
  critLog("PDKS-SYNC", "...", { userId, branchId, error: ... }, "branches.ts:N").catch(() => {});
}
```

### "Replit kendi başına kod yazmış, doğru mu?"
**Root cause:** Replit inisiyatif aldığında commit message'ları yanıltıcı olabilir.
**Tespit:** 19 Nis "Fix silent failures" başlıklı commit aslında sadece `attached_assets/` eklemiş, kod değişmemiş.
**Debug:**
```bash
git show <commit_hash> --stat        # Hangi dosyalar değişti?
git show <commit_hash> --name-only   # Sadece dosya isimleri
git diff HEAD~1 HEAD                 # Diff'i gözden geçir
```
**Kural:** Replit her commit'i sonradan Claude `git show` ile inceleyip onaylanabilir mi değerlendirir (Madde 37 §25).

## Madde 38 — Maliyet Ekonomisi & Görev Paylaşımı

DOSPRESSO üçgen mimarisi:
- **Aslan**: Karar, ürün vizyonu, kullanıcı, eğitim
- **Claude (yüksek IQ, pahalı)**: Backend, mimari, schema, API, migration, kod yazma, debug, code review
- **Replit (UI agent, ekonomik)**: Frontend UI, test, DB query, deploy, smoke test, küçük net işler

### Doğru iş bölümü:
- Yeni endpoint + yeni tablo + yeni migration → **Claude**
- Mevcut sayfa stil revizyon → **Replit**
- Backend bug fix → **Claude**
- Frontend smoke test ekran kayıtları → **Replit**
- Test senaryoları yazımı → **Claude**
- Test çalıştırma + raporlama → **Replit**

### Sıfırdan yazma yok — Paslaşma var:
Eğer Claude bir sprint'i yarım bırakmak zorunda kalırsa (oturum limiti, tool limit, vs.):
1. Ne kadar yazıldıysa **commit + push** et (eksik bile olsa)
2. Replit'e **net küçük görev** olarak devret (1 saatten az iş)
3. Replit raporlar, Claude yeni oturumda doğrular

### Sprint E Örnek (19 Nis 2026):
- **Claude**: Backend (schema + helper + 3 endpoint + 6 P0 yer integration) — 1 saat
- **Replit**: UI sayfa + route + frontend build — 1 saat
- **Toplam**: 2 saat, gereksiz Claude/Replit cost önlendi

### Replit'e iş gönderirken:
1. Önce git push yap
2. Net adım listesi (5-7 madde max)
3. Beklenen sonuç + acceptance kriterleri
4. Build komutu + test komutu
5. Rapor formatı (5 satırlık ✅/❌ tablo)
