-- ============================================================
-- DOSPRESSO — PERSONEL IMPORT PHASE 1 SQL PREVIEW
-- Tarih: 28 Nisan 2026
...
-- ⚠️  ÖNCESİNDE: pg_dump backup, bcrypt hash uygulama tarafından üret
-- 
...
--   • Phase 3 UPDATE         :  3 user (Eren, Atiye, Sema)
--   • Phase 4 INSERT users   : 35 user
--   • Phase 4b INSERT salary : 27 user (Excel'de maaş bilgisi olanlar)
...
-- 3) Aşağıdaki INSERT/UPDATE'lerde :BCRYPT_0000 → gerçek hash ile değiştir
-- ===============================================================
...
-- ════════════════════════════════════════════════════════════
-- PHASE 4 — INSERT users (35 yeni user)
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- PHASE 4b — employee_salaries history (opsiyonel, hire_date'ten itibaren)
-- ════════════════════════════════════════════════════════════
-- Excel'deki maaş bilgisi tarihçe olarak ayrı tabloya da yazılır
-- Bu sayede ileride zam/değişim takibi yapılabilir

-- [SAL-01] arifeyildirim (taban=29500, net=33500)
INSERT INTO employee_salaries (
  id, user_id, base_salary, net_salary, employment_type, effective_from,
  is_active, notes, created_at, updated_at
) SELECT
  gen_random_uuid()::text, id, 29500.00, 33500.00,
  'full_time', '2025-05-23', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 31000.00, 35000.00,
  'full_time', '2023-08-08', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 37000.00, 47500.00,
  'full_time', '2025-01-20', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 30500.00, 38500.00,
  'full_time', '2018-07-25', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 31500.00, 34500.00,
  'full_time', '2024-03-18', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 31000.00, 32500.00,
  'full_time', '2025-07-21', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 30000.00, 32500.00,
  'full_time', '2025-10-01', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 49000.00, 65000.00,
  'full_time', '2025-11-24', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 39000.00, 47000.00,
  'full_time', '2024-01-22', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 37000.00, 50000.00,
  'full_time', '2024-04-15', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 59000.00, 75000.00,
  'full_time', '2015-09-08', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 70000.00, 85000.00,
  'full_time', '2025-11-17', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 31000.00, 36000.00,
  'full_time', '2025-10-08', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 32000.00, 42000.00,
  'full_time', '2023-12-27', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 35000.00, 45000.00,
  'full_time', '2022-09-01', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 42000.00, 52000.00,
  'full_time', '2022-09-06', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 46000.00, 56000.00,
  'full_time', '2021-02-19', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 31000.00, 33000.00,
  'full_time', '2026-02-09', true,
  'Phase 1 Excel import (ozluk_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 31000.00, 43071.00,
  'full_time', '2024-09-06', true,
  'Phase 1 Excel import (lara_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 31000.00, 41256.00,
  'full_time', '2025-10-27', true,
  'Phase 1 Excel import (lara_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 31000.00, 41239.00,
  'full_time', '2025-10-27', true,
  'Phase 1 Excel import (lara_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 31000.00, 32835.00,
  'full_time', '2025-11-03', true,
  'Phase 1 Excel import (lara_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 31000.00, 31785.00,
  'full_time', '2025-12-01', true,
  'Phase 1 Excel import (lara_excel)', now(), now()
...
INSERT INTO employee_salaries (
  ...
  gen_random_uuid()::text, id, 33000.00, NULL,
  'full_time', '2026-04-22', true,
  'Phase 1 Excel import (lara_excel)', now(), now()
...
INSERT INTO employee_terminations (
  id, user_id, termination_date, termination_reason, termination_type,
  last_work_day, notes, created_at, updated_at
) SELECT
  gen_random_uuid()::text, id, '2026-02-19',
...
-- ════════════════════════════════════════════════════════════
-- Beklenen sonrası:
--   Önceki = 46 user, soft-delete edilen 36 (zaten pasifler dahil), eklenen 35
--   = 46 + 35 = 81 toplam, 35+10=45 aktif, 36+1=37 pasif/silindi
...
-- Yeni eklenen kişiler:
SELECT username, first_name, last_name, role, branch_id, hire_date
FROM users WHERE created_at >= now() - interval '5 minutes' ORDER BY branch_id, role, username;

-- Salary history kayıt sayısı (Phase 4b):
SELECT 'salary_history' AS chk, COUNT(*) AS new_rows
FROM employee_salaries WHERE created_at >= now() - interval '5 minutes';

-- Termination kayıt sayısı (Phase 5):
SELECT 'terminations' AS chk, COUNT(*) AS new_rows, MAX(termination_date) AS son_tarih
FROM employee_terminations WHERE created_at >= now() - interval '5 minutes';

-- Korunan istisnalar (hâlâ aktif olmalı, sadece UPDATE'lenenler değişti):
SELECT id, username, first_name, last_name, role, is_active, deleted_at
FROM users WHERE id IN (
  '9379801c-b971-40ed-8928-b40affbdf60e',
  '858d7987-e62e-47b9-bd9e-6c04083db8e9',
  'ee76fb82-2a78-4ae6-9792-0f483fa15636',
  '0338cc48-dfbd-4e43-b512-12e7eb92c8c4',
  '629b81fd-6ac7-4a45-8480-6e6ce3cd53b6',
...
-- ════════════════════════════════════════════════════════════
-- COMMIT vs ROLLBACK — DRY-RUN modu
-- ════════════════════════════════════════════════════════════
...
--   2) :BCRYPT_0000 → gerçek bcrypt hash ile değiştirilmeli
--   3) Phase 0 ve Phase 6 sayı çıktıları karşılaştırılmalı
--   4) Phase 4b (employee_salaries) opsiyonel — atlanabilir
--
...
--
ROLLBACK;  -- DRY-RUN modu (gerçek run için: COMMIT;)

-- ============================================================
-- DOSYA SONU — Phase 1 SQL Preview
-- 36 SD + 3 UPDATE + 35 INSERT users + ~27 INSERT salary + 1 TERMINATE
-- ============================================================