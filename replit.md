...
- **Task #288 — Wave W0 audit script reconstruction (DONE, 3 May 2026):** READ-ONLY. `scripts/audit/extract-broken-apis.mjs` committed (audit'in commitlenmemiş extraction script'i birebir methodology ile reconstruct). 624 FE × 297 server tarama → 60 distinct method+path broken (audit 118 = path×method×non-collapsed-:param expansion'ı; sayı tutarlı). Truncate band (sıra 51-60) analiz: 5 yeni kalem (NS1-NS5) W7'ye 4 + W1'e 1 dağıtıldı; 5 kalem zaten W3/W7'de mevcut. **Yeni canonical sayı: 93** (88 + 5 NS). Tam rapor: `docs/audit/broken-api-full-2026-05.md`. Reconciliation: `docs/audit/api-283-categorized-waves.md` §3.0.5-3.0.7. W0 wave dosyası DONE olarak işaretlendi.
