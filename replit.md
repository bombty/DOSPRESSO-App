...
- **Late Arrival Detection (F15 ✅ #326):** Branch-scoped tolerance dynamic via `branch_kiosk_settings.lateToleranceMinutes` — read in `payroll-engine.ts` L43/76, `routes/branches.ts` L2922, `routes/factory.ts` L1100, `routes/pdks.ts` L333. **F15 closure (#326):** `server/agent/skills/late-arrival-tracker.ts` artık `resolveLateThreshold(branchId, year, month)` helper'ı üzerinden `payrollDeductionConfig` cascade'inden okur (fabrika branchId=0 → genel cascade). Hardcoded `LATE_THRESHOLD_MINUTES=15` / `SEVERE_LATE_MINUTES=60` silindi; fallback DEFAULT_LATE_THRESHOLD_MINUTES=15 sadece DB hatası durumunda. Aylık özet (L101-150) `monthAgo` config'iyle çağrılır. Şube tarafı (L65-94) zaten tolerans karşılaştırması yapmıyor (sadece "X kişi vardiyada planlı" raporluyor) → fix kapsamı dışı. Unit test: `tests/unit/late-arrival-tracker.test.ts` (5 senaryo: db cascade, null fallback, throw fallback, diff>tol, diff<tol).
- **Şube Puantaj + Fazla Mesai Onay (#311 Bundle 7, 7/7 ✅):** Şube ayarları `branch_kiosk_settings` (schema-09) üzerinden — yeni `branch_attendance_settings` tablosu açılmadı, mevcut tablo genişletildi. Default seed `routes/branches.ts` L2586-2592 upsert ile otomatik. Onay workflow: `overtime_requests` (schema-05 L142, status `pending|approved|rejected`) + endpoint'ler `routes/misc.ts` L1182-1327 (GET/POST/approve/reject). UI: `pages/pdks.tsx` `KioskToleranceSettings` + `pages/overtime-requests.tsx`. **E2E (#327 ✅):** `tests/e2e/branch-attendance-settings.spec.ts` 3 senaryo (S1 müdür HTTP PATCH `/api/pdks/kiosk-settings` tolerans=20, S2 18dk geç+tolerans=20→`branchShiftSessions.lateMinutes` kayda girer + `attendance_penalties` 0, S3 worker POST `/api/overtime-requests` + müdür PATCH approve → status=approved + approver_id eşleşir). Gate yok, default playwright config'le çalışır. CI: `.github/workflows/e2e-bundle7.yml` yalnız manuel (workflow_dispatch); otomatik gate yok çünkü drizzle-kit push DB timeout veriyor + fresh Postgres seed pipeline ayrı task gerektirir (E2E_DATABASE_URL secret'ı seed'li DB'ye işaret etmeli). Pattern: `kiosk-shift-closure.spec.ts` (API-level Playwright + raw pg, browser yok). **Audit (#328 ✅):** ayrı maddede dokümante edildi.
...
- **Attendance Settings Audit (#328 ✅):** Şube kiosk/puantaj ayarlarının değişiklik denetim kaydı. Tablo `attendance_settings_audit` (`branch_id`, `changed_by_id`, `field_name`, `old_value`, `new_value`, `changed_at`) — schema-09 L344-363 + index `(branch_id, changed_at)`. Migration `migrations/2026-05-03-attendance-settings-audit.sql` (forward-only, idempotent, rollback = `DROP TABLE`). PATCH `/api/pdks/kiosk-settings/:branchId` artık değişen her field için 1 row insert eder (tek transaction, mevcut row SELECT → diff → bulk insert). GET `/api/branches/:id/attendance-audit?limit=50&days=90` (admin/ceo/cgo/muhasebe_ik) son 90 günü döner. UI: `pages/pdks.tsx` `AttendanceAuditList` accordion (collapsed default) `KioskToleranceSettings` kart altında. Smoke test: PATCH 15→17 → audit row inserted → GET endpoint return → restore. Runbook §A "Audit yok" kapatıldı.
