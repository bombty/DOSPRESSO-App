...
- **Task #307 — Bundle 3 Dashboard Router v4↔v5 Çakışması (DONE, 3 May 2026):** F01 çözüldü. 5 v4 dashboard route'u Centrum (v5) tek doğru yola redirect edildi (`client/src/App.tsx`): `/control` → `ROLE_CONTROL_PATH[role]`, `/merkez-dashboard` → `/muhasebe-centrum`, `/kalite-kontrol-dashboard` → `/fabrika-centrum`, `/kocluk-paneli` → `/coach-kontrol-merkezi`, `/franchise-ozet` → `ROLE_CONTROL_PATH[role]` (3 farklı yatırımcı/müdür hedefi). HomeScreen Control card zaten v5 (ROLE_CONTROL_PATH) kullanıyordu — artık her erişim yolu aynı dashboard'a düşüyor. v4 MC* dosyaları (`MissionControlHQ/Coach/Muhasebe/Supervisor/Stajyer/Fabrika/Yatirimci/Dynamic` + `DashboardRouter` + `DashboardModeToggle` + `useDashboardMode` + 5 v4-era page) artık erişilemez ölü kod — bu task'ta SİLİNMEDİ (1 task = 1 risk; ayrı follow-up cleanup task). App health 200, HMR clean.
