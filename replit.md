...
- **Key Features**:
    ...
    - **HR Management Module (2025-11-21)**: Comprehensive personnel management with özlük dosyası (personnel documents), disciplinary tracking, and employee onboarding. Backend: 30+ storage methods, 20+ API endpoints for documents, disciplinary reports, and onboarding. Frontend: Personel Yönetimi list page with search/filters, Personel Detay page with 3 tabs. **Özlük Dosyası**: Document upload, HQ verification, delete. **Disiplin İşlemleri**: Create reports (reportType, severity, subject, description, incident details), add employee responses, resolve with action taken. **Onboarding**: Status tracking (completion %, dates), task management (create, complete, HQ verify), **onboarding auto-create** feature with idempotent `getOrCreateEmployeeOnboarding()` backend method ensuring one-click onboarding start via "Onboarding Başlat" button with full Zod validation, assignedById tracking, and proper error handling via apiRequest helper. Component architecture: Extracted dialogs to `client/src/components/hr/` (DisciplinaryDialogs.tsx, OnboardingTaskDialog.tsx) for maintainability. Permissions: Supervisor (view, create, edit, delete), Coach/Admin (full access + approve). Comprehensive data-testid coverage for automated testing.
    - **Training Academy Dashboard Integration (2025-11-21)**: Enhanced dashboard with dedicated Akademi card showing training stats (mandatory count, in-progress, completed, completion percentage) with loading skeleton and empty-state messaging. Sidebar Akademi menu item accessible to all users with auto-generated data-testid (`link-akademi`). Training page "Benim Eğitimlerim" tab now displays both mandatory trainings (with yellow star and "Zorunlu" badge) AND optional trainings where user has started progress, enabling comprehensive personal training tracking. HQ role-based training assignment UI with multi-select checkbox grid for assigning trainings as mandatory to specific roles via `requiredForRole` field. Design decisions: Mandatory trainings prioritized in user view, optional trainings discoverable when user begins progress, comprehensive data-testid coverage for automated testing.
