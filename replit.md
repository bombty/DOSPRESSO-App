...
- **Task #285 — 70 kırık API path kategorize raporu (MERGED, 2 May 2026):** READ-ONLY analiz. `docs/audit/api-283-categorized-waves.md` v2 üretildi. APP_AUDIT Bölüm 7.1 truncate olduğu için (audit script repo'da yok) bağımsız extraction script `.local/scripts/audit-tmp/extract2.mjs` yazıldı: 717 distinct FE path × 1614 server path × 12 mount-prefix → 4 aşamalı eşleştirme (direct, mount, :param missing, sub-path). **Sonuç: 70 path kırık/kısmi** (audit'in 118'i method-bazlı saydığı için 70 × ~1.7 method ≈ 118 ile tutarlı). Kategori dağılımı: %66 (46) FE patch (a1 :param eksik 35, a2 sub-path 6, a4 method mismatch 5), %34 (24) gerçek server-yok veya dead. **7 modül-bazlı dalga (W1-W7):** W1 FACTORY (13), W2 BRANCH+DASHBOARD (11), W3 HR (6), W4 OBJECT_STORAGE konsolidasyon (4), W5 CRM+AUTH+ACADEMY+AGENT+OPS (16), W6 ADMIN (8), W7 DİĞER (12) = 70. Toplam ~37 saat. Risk-bazlı sıra: W3+W6+W4 öncelikli. **DRIFT:** Method mismatch tam taraması yapılmadı; Dalga 0 (method+path eşleme script fix) ayrı task olarak owner kararı sonrası propose edilecek. Architect re-review v2.1: PASS.
