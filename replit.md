...
- **Security & Bug Fix Sprint 9**: (1) `setIsAiOnboardingOpen` scope fix — passed as props to `RoleDashboardSection` in `academy-hq.tsx`. (2) Circular dependency fix — `megaModuleConfig.ts` uses Proxy-based lazy initialization for `MEGA_MODULE_CONFIG` and `MEGA_MODULE_ORDER`. (3) Session timeout reduced from 7 days to 8 hours (1 shift). (4) Login lockout — in-memory Map: 5 failed attempts in 15 min → 15 min lock + admin critical notification. (5) Password complexity enforced across all 6 validation points (server: routes.ts register/reset, hr.ts reset, admin.ts create; client: register.tsx, reset-password.tsx) — min 8 chars, 1 uppercase, 1 lowercase, 1 digit, 1 special char. E2E verified: login, lockout, password rejection, mega-module navigation, Academy page all passing.
