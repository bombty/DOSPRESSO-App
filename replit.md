...
- **Task #285 — 80 kırık method+path kategorize raporu (IN REVIEW, 2 May 2026):** READ-ONLY analiz. `docs/audit/api-283-categorized-waves.md` v3 üretildi. APP_AUDIT Bölüm 7.1 truncate olduğu için bağımsız extraction yapıldı: (a) `extract2.mjs` path-bazlı 70 broken; (b) `extract3-method.mjs` + `extract4-expand.mjs` method-aware: 1352 method+path FE call → 732 matched → 71 path-expansion + 9 yeni method-mismatch keşfi (path mevcut ama method yanlış: `/api/mrp/leftovers` GET vs POST, `/api/onboarding-tasks`, `/api/fault-service-tracking`, `/api/service-requests`, `/api/salary/employee`, `/api/staff-evaluations`, `/api/notifications`, `/api/training/assignments`, `/api/feedback-custom-questions`) = **80 method+path satır**. Audit'in 118 sayısı (script kayıp olduğu için doğrulanamadı) muhtemelen daha agresif normalize sonucu false-positive içeriyor; 80 = yüksek-güven alt küme. **7 modül-bazlı wave skeleton dosyası** üretildi (`.local/tasks/wave-W{1..7}-*.md`): W1 FACTORY (13+1MM), W2 BRANCH+DASHBOARD (11), W3 HR (6+3MM), W4 OBJECT_STORAGE konsolidasyon (4), W5 CRM+AUTH+ACADEMY+AGENT+OPS (16+2MM), W6 ADMIN (8), W7 DİĞER (12+3MM) = 70 path + 9 MM = 80. Toplam ~37 saat. Risk-bazlı sıra: W3+W6+W4 öncelikli. Wave dosyaları her birinde Status, Mode, Süre, Risk, Acceptance, Paralel-güvenlik içerir; Task #283.1-#283.7 olarak owner GO sonrası project_tasks'a propose edilecek.
- **Sırada:** Task #286 (B12 kiosk vardiya E2E test) + Task #287 (B4 ay sonu puantaj sim) PENDING. Task #283 dalgaları (Dalga 0-7) ayrı task'lar olarak owner kararı sonrası propose edilecek.
