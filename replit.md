...
- **Task #285 — 118 kırık API çağrısı kategorize raporu (MERGED, 2 May 2026):** READ-ONLY analiz. `docs/audit/api-283-categorized-waves.md` üretildi. Kritik bulgular: (M1) audit script mount-prefix bug — `/api/iletisim/*` 6 endpoint hepsi MEVCUT (false positive); (M2) route :param eşleme hatası — 12 endpoint server'da `:id/:userId/:date/:branchId` ile var ama FE base path çağırıyor; (M3) HTTP method mismatch — `/api/auth/logout` POST var, audit GET tarıyor; (M4) Object Storage scattering — 6 farklı upload endpoint'i konsolidasyon ihtiyacı. İlk 50 endpoint tam kategorize (architect review sonrası düzeltilmiş): ~%55-65 FE patch (kategori a), ~%24-28 audit script bug (a-script: M1 mount + M3 method), ~%8-12 gerçek server-yok (b — sadece 4 Object Storage path), ~%0-5 dead. **Kalan 68 endpoint için DRIFT:** audit raporu Bölüm 7.1 sadece ilk 50'yi listeliyor; tam liste için Dalga 0 (script fix + re-run) zorunlu. 8 paralel-güvenli dalga tarif edildi.
