...
- **Employee Summary Service:** Aggregates per-employee performance metrics (training completion, quiz scores, tasks, branch tasks, attendance) into weighted composite scores. Endpoints: `GET /api/employee-summary/:userId`, `GET /api/employee-summary/branch/:branchId`, `GET /api/employee-summary/branch/:branchId/quick`. Role-based access: HQ roles see all, branch managers see own branch only.
