...
- **Task #288 — Wave W0 audit script reconstruction (DONE v2, 3 May 2026):** READ-ONLY. `scripts/audit/extract-broken-apis.mjs` committed. v1: parsing bug (`?` split before template collapse) → architect REJECTED. v2 düzeltmeleri: (1) `collapseTemplates()` helper, collapse-then-split sırası — `${ticket?.id}` truncate fix; (2) `isQueryArtifactMatch()` — `${qs}` artifact false positive fix; (3) raw audit-style view emission. **Final: 51 distinct broken** (missing=2, mm=7, rel=42). Audit 118 = raw expansion (`:param` substitute YAPMADAN her FE template literal varyantı ayrı satır); raw view bunu birebir reproduce ediyor. Önceki 88/93 stale — wave dosyaları (W1-W7) v2 ile reconciliation pilot için low-priority follow-up. Tam rapor: `docs/audit/broken-api-full-2026-05.md` (collapsed + raw view). Reconciliation: `docs/audit/api-283-categorized-waves.md` §3.0.5-3.0.7.
